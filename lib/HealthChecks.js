...
module.exports = class HealthChecks extends Function {
	...
	constructor(opts = {}) {
		...
		if (opts.updatedHealthChecks) {
			opts.liveness = opts.liveness || '/_ah/liveness'
			opts.readiness = opts.readiness || '/_ah/readiness'
		}
		...
		for (const ep of [this._healthy, this._alive, this._ready]) {
			...
			if (handled !== false) {
				...
				if (handled) {
					...
					const statusCode = err.statusCode || (typeof err.code === 'number' ? err.code : null) || 500
