...
function makeLogger(forGAE, logName) {
	...
	if (forGAE) {
		...
		if (process.env.LOGGING_INSTANCE_LABEL) sdOptions.labels.instance = process.env.LOGGING_INSTANCE_LABEL

		if (process.env.LOGGING_RESOURCE) {
			sdOptions.resource = JSON.parse(process.env.LOGGING_RESOURCE)
		} else {
			if (process.env.GCE_INSTANCE_GROUP) {
				delete sdOptions.labels.instance
		...
		console.info('Using StackDriver config:', JSON.stringify(sdOptions, null, 2))
		return createWinstonLogger({
			...
			format: format.combine(
				...
				format(info => {
					...
					if (args.length > 0) info.message += util.format('', ...args.map(a => util.inspect(a, { depth: 10 })))
					if (String(last) === String({})) {
						Object.assign({}, last, info)
					} else {
						last = last !== undefined ? util.format('', util.inspect(last, { depth: 10 })).trim() : ''
			...
			format.printf(info => {
				...
				if (message || Symbol.for('splat') in info) {
					...
					if (Symbol.for('splat') in info) {
						const splat = info[Symbol.for('splat')]
						const splatM = util.format('', ...(Array.isArray(splat) ? splat : [splat]).map(s => util.inspect(s, {
							depth: 10,
							colors: true
						}))).trim()
