...
module.exports = class SubscriptionInfo {
	...
	startListening() {
		logger.debug(`Start monitoring subscription: ${this._subName}`)
	...
	stopListening() {
		logger.debug(`Stop monitoring subscription: ${this._subName}`)
	...
	_check(nowDate) {
		if (!this.startedListeningAtDate) {
			logger.error('Not yet started listening to subscription:', this._subName, this)
			...
			if (this.lastMessageDate) {
				...
				if (this.lastMessageDate.getTime() < lastMessageMin) {
					...
					logger.error('Subscription alive check failed:', this._subName, this)
				...
				if (this.startedListeningAtDate.getTime() < lastMessageMin) {
					logger.error('Subscription never received a message:', this._subName, this)
	...
	_messageListener(message) {
		logger.verbose(`Received message (sub: ${this._subName}) age - ${utils.ageOfMessage(message)} [id=${message.id}]`, message)
		
		logger.debug(`Last received a message for ${this._subName} at: ${this.lastMessageDate}`)
	...
	_errorListener(err) {
		logger.verbose(`Received error (sub: ${this._subName}):`, err)
