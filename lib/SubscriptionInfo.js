...
module.exports = class SubscriptionInfo {
	 ...
	 */
	constructor(sub, maxQuietPeriodMs, restartHandler) {
	...
	_restart() {
		...
		return new Promise((resolve, reject) => {
			self.restartHandler(self._sub, (restartErr, newSub) => {
				if (restartErr) {
					logger.error(`Failed to re-create subscription: ${self._subName}`, restartErr)
					return reject(restartErr)
				}
				if (newSub === self._sub) return resolve() // Sub not changed
				if (newSub) logger.info(`Re-created subscription: ${self._subName}`)
